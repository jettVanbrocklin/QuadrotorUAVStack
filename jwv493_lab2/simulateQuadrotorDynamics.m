...
Xbig = [S.state0.r',S.state0.v',RBI(1,1),RBI(2,1),RBI(3,1),RBI(1,2),RBI(2,2),RBI(3,2),RBI(1,3),RBI(2,3),RBI(3,3),S.state0.omegaB']';
for k=1:N-1
    tspan = [S.tVec(k):dtOut:S.tVec(k+1)]';
    omegaVec = S.omegaMat(k,:)'; % Transpose so it is a column vector;
    distVec = S.distMat(k,:)';
    [tVeck,XMatk] = ode45(@(t,X)quadOdeFunction(t,X,omegaVec,distVec,parameters), tspan, Xbig);
    % Store Values
